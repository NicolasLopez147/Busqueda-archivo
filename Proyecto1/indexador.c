...
int main() {
    ...
    if ((entrada = fopen("archivo.csv", "rt")) == NULL || (salidaIndex = fopen("salidaIndex", "wb+")) == NULL
                                                      || (salidaHash = fopen("salidaHash", "wb+")) == NULL) {
    ...
    if(fgets(linea, tamano, entrada) == NULL) {         // Se salta la primera linea del .csv
    ...
    while( fgets(linea, tamano, entrada) != NULL ) {
        ...
        if(token != NULL) {
            ...
            if( tablaHash[idHash].idOrigen == -1 ) {                   // Es el primer registro con determinado idOrigen
